-- Sigma Auto-ID + Compact Registration (Modern Chat)
...
-- HEADUSERNAME LOGIC
if LocalPlayer.Name == shared.HEAD_USER_NAME then
    ...
    -- After 5s, announce all registered bots in ONE message
    task.delay(5, function()
        local list = {}
        for botName in pairs(shared.verifiedBots) do
            table.insert(list, botName)
        end
        table.sort(list) -- consistent ordering
...
-- BOT LOGIC
    ...
    generalChannel.OnIncomingMessage = function(message)
        ...
        if textSource and message.Text then
            ...
            if success and speaker == shared.HEAD_USER_NAME then
                -- Step 1: Get the code
                ...
                -- Step 2: Detect registration (single compact message)
                local regMessage = message.Text:match("^%[HEAD%] Registered: (.+)")
                if regMessage then
                    -- Find the bot's number + confirm registration
